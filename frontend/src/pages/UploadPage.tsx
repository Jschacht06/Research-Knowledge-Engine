...
export function UploadPage() {
  ...
  const availableTopics = useMemo(
    () => Array.from(new Set([...documents.flatMap((document) => document.topics), ...topics])).sort(),
    [documents, topics],
  )
      ...
      <div className="space-y-6 rounded-[32px] border border-rke-border/80 bg-white p-8 shadow-[0_24px_70px_rgba(24,46,75,0.08)]">
        ...
        <div>
          ...
          {topics.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {topics.map((topic) => (
                <span key={topic} className="inline-flex items-center gap-2 rounded-full bg-rke-teal-soft px-3 py-2 text-sm text-rke-navy">
                  {topic}
                  <button className="text-rke-copy transition hover:text-rke-navy" onClick={() => toggleTopic(topic)} type="button">
                    <X size={14} />
                  </button>
                </span>
              ))}
